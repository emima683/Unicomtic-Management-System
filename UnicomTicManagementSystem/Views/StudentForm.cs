...
namespace UnicomTicManagementSystem.Views
{
    public partial class StudentForm : Form
    {
       private StudentController studentController = new StudentController();
        ...
        {
            if (string.IsNullOrWhiteSpace(lblF_Name.Text)|| string.IsNullOrWhiteSpace(lblL_Name.Text)|| string.IsNullOrWhiteSpace(lblAddress.Text))
            {
        ...
        private void btn_Update_Click(object sender, EventArgs e)
        {
            if(Student_DGV.SelectedRows.Count==0)
            {
                ...
                if (string.IsNullOrWhiteSpace(lblF_Name.Text) || string.IsNullOrWhiteSpace(lblL_Name.Text) || string.IsNullOrWhiteSpace(lblAddress.Text))
            {
                MessageBox.Show("please Enter the First_Name, Last_Name and Address.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return; 
            }
...

            {
                ...
                studentController.UpdateStudent(stu_id, student);              
                get_student_info();
            }
        }
